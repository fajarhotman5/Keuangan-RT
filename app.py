...
# ==========================================
    ...
    if df_trans.empty:
        ...
        id_terpilih = st.session_state.get('id_terpilih_riwayat', None)
        
        # --- MODE DESKTOP / LAPTOP (Sesuai Gambar 1 - Tetap Asli Tanpa Diubah) ---
        st.markdown('<div class="desktop-only">', unsafe_allow_html=True)
        if not df_tampil.empty:
            # Dropdown aksi cepat orisinal untuk PC tetap dipertahankan penuh
            aksi_cepat_pc = st.selectbox(
                "⚡ Aksi Cepat Transaksi (Pilih Kategori):", 
                ["-- Pilih Transaksi untuk Edit / Hapus --"] + [f"ID {r['id_transaksi']} - {r['kategori']} (Rp {r['jumlah']:,.0f})" for _, r in df_tampil.iterrows()],
                key="aksi_pc"
            )
            
            if aksi_cepat_pc != "-- Pilih Transaksi untuk Edit / Hapus --":
                id_terpilih = int(aksi_cepat_pc.split(" - ")[0].replace("ID ", ""))
            
            st.markdown("""
                <table style='width:100%; border-collapse: collapse; font-size: 12px; text-align: left;'>
                    <tr style='background-color: var(--background-color); border-bottom: 2px solid #8B0000;'>
                        <th style='padding: 8px; color: var(--text-color);'>Tanggal</th>
                        <th style='padding: 8px; color: var(--text-color);'>Aliran</th>
                        <th style='padding: 8px; color: var(--text-color);'>Wallet</th>
                        <th style='padding: 8px; color: var(--text-color);'>Kategori</th>
                        <th style='padding: 8px; color: var(--text-color);'>Nominal</th>
                        <th style='padding: 8px; color: var(--text-color);'>Keterangan</th>
                    </tr>
            """, unsafe_allow_html=True)
            ...
            for index, row in df_tampil.iterrows():
                tgl_str = row['tanggal'].strftime('%d-%m-%Y')
                txt_jenis = "Masuk" if row['jenis'] == "Pemasukan" else "Keluar"
                color_jenis = "#2E7D32" if row['jenis'] == "Pemasukan" else "#C62828"
                bg_row = "rgba(139,0,0,0.05)" if id_terpilih == row['id_transaksi'] else "transparent"
                
                st.markdown(f"""
                    <tr style='background-color: {bg_row}; border-bottom: 1px solid rgba(0,0,0,0.1);'>
                        <td style='padding: 8px; color: var(--text-color);'>{tgl_str}</td>
                        <td style='padding: 8px; color: {color_jenis}; font-weight: bold;'>{txt_jenis}</td>
                        <td style='padding: 8px; color: var(--text-color);'>{row['wallet']}</td>
                        <td style='padding: 8px; color: var(--text-color); font-weight: 600;'>{row['kategori']}</td>
                        <td style='padding: 8px; color: {color_jenis}; font-weight: bold;'>Rp {row['jumlah']:,.0f}</td>
                        <td style='padding: 8px; color: var(--text-color); opacity: 0.8;'>{row['keterangan'] or '-'}</td>
                    </tr>
                """, unsafe_allow_html=True)
        ...
        # --- MODE MOBILE / HP (Sesuai Gambar 2 Dengan Upgrade Keren) ---
        ...
        if not df_tampil.empty:
            for index, row in df_tampil.iterrows():
                ...
                # Desain Baru: Nilai Geser Ke Tengah, Kanan Bawah untuk Tombol Aksi Mandiri
                st.markdown(f"""
                    <div style='background-color: var(--background-color); border: 1px solid rgba(139,0,0,0.2); padding: 12px; border-radius: 8px; margin-bottom: 8px; position: relative;'>
                        ...
                        <div style='font-size: 15px; font-weight: 800; text-align: center; color: {badge_color}; margin-top: 5px; margin-bottom: 5px;'>
                            Rp {row['jumlah']:,.0f} <span style='font-size: 9px; font-weight: normal; color: var(--text-color); opacity: 0.4;'>(ID: {row['id_transaksi']})</span>
                        </div>
                    </div>
                """, unsafe_allow_html=True)
                
                # Button aksi diletakkan sejajar rapi di bawah masing-masing kartu
                col_hp1, col_hp2 = st.columns(2)
                with col_hp1:
                    if st.button(f"📝 Edit", key=f"edit_hp_{row['id_transaksi']}", use_container_width=True):
                        st.session_state.id_terpilih_riwayat = row['id_transaksi']
                        st.session_state.sub_aksi_hp = "edit"
                        st.rerun()
                with col_hp2:
                    if st.button(f"🗑️ Hapus", key=f"hapus_hp_{row['id_transaksi']}", use_container_width=True):
                        st.session_state.id_terpilih_riwayat = row['id_transaksi']
                        st.session_state.sub_aksi_hp = "hapus"
                        st.rerun()
                        
            # Handler penentu menu modal dari klik tombol kartu HP
            if st.session_state.get('id_terpilih_riwayat') and st.session_state.get('sub_aksi_hp'):
                id_terpilih = st.session_state.id_terpilih_riwayat
        else:
            st.write("Data tidak ditemukan.")
        st.markdown('</div>', unsafe_allow_html=True)
        
        # --- FORM MODAL EXECUTOR (EDIT / HAPUS) ---
        if id_terpilih and id_terpilih in df_trans['id_transaksi'].values:
            data_row = df_trans[df_trans['id_transaksi'] == id_terpilih].iloc[0]
            
            # Tentukan paksa sub-mode jika lewat PC default-nya adalah edit
            sub_mode = st.session_state.get('sub_aksi_hp', "edit")
            
            st.markdown("---")
            if sub_mode == "edit":
                st.markdown(f"<p style='color: #B8860B; font-weight: bold; font-size: 13px; margin-bottom: 4px;'>🛠️ Edit Transaksi (ID: {id_terpilih})</p>", unsafe_allow_html=True)
                    ...
                    with col_ef1:
                        if st.form_submit_button("Simpan Perubahan", use_container_width=True):
                            try:
                                ...
                                conn.commit()
                                conn.close()
                                if 'id_terpilih_riwayat' in st.session_state: del st.session_state.id_terpilih_riwayat
                                if 'sub_aksi_hp' in st.session_state: del st.session_state.sub_aksi_hp
                ...
                st.markdown(f"<p style='color: #8B0000; font-weight: bold; font-size: 13px; margin-bottom: 4px;'>🗑️ Hapus Transaksi (ID: {id_terpilih})</p>", unsafe_allow_html=True)
                st.markdown(f"<div style='background-color:rgba(198,40,40,0.1); padding:10px; border-radius:6px; border:1px solid #c62828; margin-bottom:8px; font-size:11px; color:#c62828;'>Hapus transaksi data <b>{data_row['kategori']} (Rp {data_row['jumlah']:,.0f})</b> secara permanen?</div>", unsafe_allow_html=True)
